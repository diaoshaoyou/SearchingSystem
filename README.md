...
2、再将新字符转换成2-gram去查找索引

3、对每个词项进行检查把不符合的词项去除

==展示后改进==：一开始做的时候把转换成2-gram的词都进行了AND操作，导致系統在处理字符的AND操作太过于久，导致用了整个数据库的时候，查询过程非常慢（**大概2-3分钟**）。

后来发现是在处理字符太慢，所以就不进行AND操作，把所有有可能的词项都存起來，直接对每个词项进行检查然后保留，就不用对处理字符的AND操作。改进后查询时间降为**1-2秒**。

#### 8、拼写矫正

1、利用2-gram去查找相关词项

2、对每个2-gram索引进行AND操作找出重复的次数

3、在判定中增加了矫正字符长度和结果字符长度不超过1

4、最后对重复次数最高的那些词项对它计算编辑距离

5、输出编辑距离最小的词项

#### 9、基于快速评分的Top K查询

1、对每个文件遍序所有词项计算查詢文件和每个文件的余弦相似值
